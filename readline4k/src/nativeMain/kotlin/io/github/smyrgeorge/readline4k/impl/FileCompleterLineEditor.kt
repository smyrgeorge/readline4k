...
class FileCompleterLineEditor(
    ...
    override val rl: COpaquePointer = memScoped {
        ...
        new_file_completer_editor_with_config(cfg.ptr) ?: couldNotInstantiateTheEditor()
    }

    override fun readLine(): Result<String> = file_completer_editor_read_line(rl, linePrefix).toStringResult()
    override fun loadHistory(path: String): Result<Unit> {
        ...
        else file_completer_editor_load_history(rl, path).toUnitResult()
    }

    override fun addHistoryEntry(entry: String) = file_completer_editor_add_history_entry(rl, entry)
    override fun saveHistory(path: String): Result<Unit> = file_completer_editor_save_history(rl, path).toUnitResult()
    override fun clearHistory(): Result<Unit> = file_completer_editor_clear_history(rl).toUnitResult()
}